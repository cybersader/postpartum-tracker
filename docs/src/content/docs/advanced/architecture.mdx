...
Plugin (main.ts)
  ...
  └── Code block processor
        └── TrackerWidget      MarkdownRenderChild
              ...
              ├── QuickActions
              ├── AlertsPanel
              └── CollapsibleSection[] (one per module)
                    └── module.buildUI()
```
...
Entries are always sorted by timestamp after adding or editing, so they display chronologically.
...
src/
  ...
  notifications/
    NotificationService.ts          Periodic scanner
