...
4. **Add fields** -- define custom data fields for each entry:
   ...
   :::tip
   For duration trackers, set "Collect on" to `stop` for fields like mood or notes that make more sense after the activity ends. Use `start` for fields you want captured before the timer begins.
   :::
